...
void solve() {
    ll n; read(n);
    vector<string> arr(n); read_array(n, arr);
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < str.size(); j++) {
            string first = str.substr(0, j), second = str.substr(j);
            if (uniq.count(first) and uniq.count(second)) ans[i] = '1';
        }
    }
    cout << ans << endl;
}
