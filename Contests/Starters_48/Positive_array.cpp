...
url: https://www.codechef.com/START48B/problems/CIREQ
interactive: false
memoryLimit: 256
timeLimit: 1000
Started At: 8:04:47 PM
*/
#include <bits/stdc++.h>
using namespace std;
...
void solve() {
    ...
    ll cnt = 0;
    multiset<ll> ms;
    for (auto &a : arr) ms.insert(a);


    while (ms.size()) {
        int j = 0;
        auto get = ms.upper_bound(j);
        while (ms.size() and get != ms.end()) {
            j++;
            ms.erase(get);
            get = ms.upper_bound(j);
        }
        cnt++;
    }
    cout << cnt << endl;
