...
void solve() {
    ...
    for (int i = 1; i < n; i++) {
        ll diff = H[i - 1] - H[i];
        pref[i] += pref[i - 1] + (diff > 0 ? diff : 0);
    }
    for (int i = n - 2; i >= 0; i--) {
        ll diff = H[i + 1] - H[i];
        suff[i] += suff[i + 1] + (diff > 0 ? diff : 0);
    }
    ...
    while (m--) {
        ...
        if (start < end) {
            ll sub = start - 1 >= 0 ? pref[start - 1] : 0;
            ans = pref[end] - pref[start];
        }
        else {
            ll sub = start + 1 < n ? suff[start + 1] : 0;
            ans = suff[end] - suff[start];
        }
        cout << ans << endl;
    }
