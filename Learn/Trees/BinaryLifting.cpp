 ...
 * repeat k times:
 * v = parent[v]
 * Worst case complexity -: O(Q*N)
 *
 * If we are able to answer the query in O(very small time) when k
 * is power of 2, then we can decrease the complexity.
*/
 ...
 * Time and space Complexity -> O(N*log(N))
 ...
 # If the order of nodes is not as per the condition
    for v = 0..N -1:
        up[v][0] = parent[v]
    for j=1--LOG -1
        for v=0..N-1
