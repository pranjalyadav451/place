/*
name: D - Trophy
group: AtCoder - AtCoder Beginner Contest 258
url: https://atcoder.jp/contests/abc258/tasks/abc258_d
interactive: false
memoryLimit: 1024
timeLimit: 2000
Started At: 10:39:33 AM
*/
#include <bits/stdc++.h>
using namespace std;
...
void solve() {
    ...
    ull min_game_time = ULONG_LONG_MAX;

    for (int i = 0; i < n; i++) {
        read(arr[i].first, arr[i].second);
        min_game_time = min(min_game_time, arr[i].second);
    }
    ull min_time = ULONG_LONG_MAX;
    ull time_passed = 0;

    for (int i = 0; i < n; i++) {
        time_passed += arr[i].first + arr[i].second;
        min_time = min(min_time, time_passed + (x - (ull)1) * arr[i].second);
        x--;
        if ((x) == 0) break;
    }
